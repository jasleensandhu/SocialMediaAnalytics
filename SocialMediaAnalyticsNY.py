...
class MyStreamer(TwythonStreamer):
    ...
    def on_success(self, data):
        if 'lang' in data and data['lang'] == 'en' and ('trump' in data['text'] or 'Trump' in data['text']):
            ...
            client = MongoDB()
            countTweetsQuery = client.get_collection('newyork').count()
    ...
    def upload_data(self, postData):
        client = MongoDB()
        tweetsCollection = client.get_collection('newyork')
    ...
    def create_word_cloud(self):
        ...
        cachedStopWords.update(('well', 'say', 'New', 'RT', 'now', 'https', 'via', 'CIA', 'make','says','new','will','said',
                                'take', 'amp', 'one','go', 'know', 'day', 'look', 'think', 'link', 'need', 'going', 'PussyHats',
                                'us', 'CNN', 'much', 'want'))
    ...
    def nmf_topic_modeling(self):
        ...
        cachedStopWords.update(
            ('well', 'say', 'New', 'RT', 'now', 'https', 'via', 'CIA', 'make', 'says', 'new', 'will', 'said',
             ...
             'cia', 'elizasoul80', '5febckimwt', 'af', 'ex', 'pussyhats', 'jkksztxvt0', 'aftr', 'ekzz3kwl0o', 'houseintelcomm',
             'realdonaldtrump', 'senwarren', 'mitchellvii', 'us', 're', 'much', 'ruse', 'loudobbs', 'intel'))
    ...
    def store_json(self):
        with open('tweet_stream_{}_{}.json'.format('newyork', len(tweets)), 'w') as f:
