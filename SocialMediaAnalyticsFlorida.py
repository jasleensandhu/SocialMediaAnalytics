...
class MyStreamer(TwythonStreamer):
    ...
    def on_success(self, data):
        if 'lang' in data and data['lang'] == 'en' and ('trump' in data['text'] or 'Trump' in data['text']):
            ...
            client = MongoDB()
            countTweetsQuery = client.get_collection('florida').count()
    ...
    def create_word_cloud(self):
        ...
        cachedStopWords.update(('well', 'say', 'New', 'RT', 'now', 'https', 'via', 'CIA', 'make','says','new','will','said',
                                'take', 'amp', 'one','go', 'know', 'day', 'look', 'think', 'still', 'right', 'way',
                                'done', 'please', 'realDonaldTrump', 'ACA', 'GOP', 'must', 'want', 'gt', 'need', 'us',
                                'much', 'see', 'Dem', 'thing'))
    ...
    def store_json(self):
        with open('tweet_stream_{}_{}.json'.format('florida', len(tweets)), 'w') as f:
