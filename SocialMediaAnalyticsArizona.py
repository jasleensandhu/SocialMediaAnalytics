...
class MyStreamer(TwythonStreamer):
    ...
    def on_success(self, data):
        if 'lang' in data and data['lang'] == 'en' and ('trump' in data['text'] or 'Trump' in data['text']):
            ...
            client = MongoDB()
            countTweetsQuery = client.get_collection('arizona').count()
    ...
    def upload_data(self, postData):
        client = MongoDB()
        tweetsCollection = client.get_collection('arizona')
    ...
    def create_word_cloud(self):
        ...
        cachedStopWords.update(('well', 'say', 'New', 'RT', 'now', 'https', 'via', 'CIA', 'make','says','new','will','said',
                                'take', 'amp', 'one','go', 'know', 'day', 'look', 'think', 'us', 'much', 'ACA', 'way', 'need',
                                'see', 'UN', 'GOP', 'still', 'needs', 'MUST','OPEN'))
    ...
    def nmf_topic_modeling(self):
        ...
        cachedStopWords.update(
            ('well', 'say', 'New', 'RT', 'now', 'https', 'via', 'CIA', 'make', 'says', 'new', 'will', 'said',
             ...
             'cia', 'elizasoul80', '5febckimwt', 'af', 'ex', 'realdonaldtrump', 're', 'mikepencevp', 'un', 'recv', 'cmmte',
             'gop', 'devinnunes', 'ahca', 'want', 'see', 'aca', 'us', 'mr', 'gorsuch', 'much', 'way'))
    ...
    def store_json(self):
        with open('tweet_stream_{}_{}.json'.format('arizona', len(tweets)), 'w') as f:
