...
class MongoDB():
    def __init__(self):
        client = MongoClient('mongodb://ds113650.mlab.com:13650')
        self.db = client['socialmediaanalyticsdb']
...
class MyStreamer(TwythonStreamer):
    ...
    def on_success(self, data):
        if 'lang' in data and data['lang'] == 'en' and ('trump' in data['text'] or 'Trump' in data['text']):
            ...
            client = MongoDB()
            countTweetsQuery = client.get_collection('california').count()
    ...
    def upload_data(self, postData):
        client = MongoDB()
        tweetsCollection = client.get_collection('california')
    ...
    def create_word_cloud(self):
        ...
        cachedStopWords.update(('well', 'say', 'New', 'RT', 'now', 'https', 'via', 'CIA', 'make','says','new','will','said',
                                'take', 'amp', 'one','go', 'know', 'day', 'look', 'think', 'ZApA754BvC', 'Donald', 'wvnVOEvrhT',
                                'need', 'VIDEO'))
    ...
    def nmf_topic_modeling(self):
        ...
        cachedStopWords.update(
            ('well', 'say', 'New', 'RT', 'now', 'https', 'via', 'CIA', 'make', 'says', 'new', 'will', 'said',
             ...
             'cia', 'elizasoul80', '5febckimwt', 'af', 'ex', 'zapa754bvc', 'wvnvoevrht', 're', 'realdonaldtrump', 'un',
             'doesn', 'trumpcare', 'gop', 'us', 'let', 'em8ramofzg', 'donaldjtrumpjr', 'russian', 'jr'))
    ...
    def lda_topic_modeling(self):
        ...
        cachedStopWords.update(
            ('well', 'say', 'new', 'rt', 'now', 'https', 'via', 'cia', 'make', 'says', 'new', 'will', 'said', 'take','amp',
             ...
             'https://t.co/tmurxjv5a4', 'is', 'you', 'we', 'be', 'ur'))
    ...
    def store_json(self):
        with open('tweet_stream_{}_{}.json'.format('california', len(tweets)), 'w') as f:
